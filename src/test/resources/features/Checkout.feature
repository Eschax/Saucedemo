...
#     And     Buyer will see checkout overview p1 <productname> and p2 <productname2> on the page
...
#     |standard_user  |secret_sauce    |Sauce Labs Fleece Jacket   |Jhon         |Wick         | 12345         |
    ...
    Given   Buyer login to the saucedemo username <username> and password <password>
    When    Buyer add a product <productname> to the cart
    And     Buyer checkout product1 <productname>
    And     Buyer remove product1 <productname> from the cart
    And     Buyer back to home to change product
    And     Buyer add a new product <productname2> to the cart
    And     Buyer checkout new product <productname2>
    And     Buyer insert <Firstname> <Lastname> <PostalCode> for shipping
    And     Buyer see checkout overview new product <productname2>
    Then    Buyer will see message confirmation page Thank you for your order!
    ...
    # |problem_user   |secret_sauce    |standard_user   |secret_sauce    |Sauce Labs Backpack   |Jhon         |Wick        | 12345        |